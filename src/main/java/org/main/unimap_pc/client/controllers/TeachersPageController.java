...
public class TeachersPageController implements LanguageSupport {
    ...
    @FXML private Label navi_username_text, navi_login_text, ais_id_teach, name_teach, room_teach, teach_list;
    ...
    private void applyFilters() {
        ...
        FilterService.teacherSearchForm.roleEnum roleEnum = convertStringToRoleEnum(role);
        ...
        Logger.info("Filtered teachers: " + filteredTeachers.size());
        updateTeacherList(filteredTeachers);
        updateSelectedFiltersText();
    }

    private FilterService.teacherSearchForm.roleEnum convertStringToRoleEnum(String role) {
        return switch (role) {
    ...
    private void updateSelectedFiltersText() {
        searchField.setStyle(searchField.getText().trim().isEmpty() ? "" : "-fx-border-color: #1976D2;");
        roleCombo.setStyle(roleCombo.getValue().equals("All Semesters") ? "-fx-text-fill: black;" : "-fx-text-fill: #1976D2;");
    }

    private void updateTeacherList(List<Teacher> teachers) {
        anchorScrollPane.getChildren().clear();

        // Container for the teacher list
        ...
        if (teachers.isEmpty()) {
            displayNoTeachersMessage(teacherContainer);
        } else {
            displayTeachers(teachers, teacherContainer);
        }
    ...
    private void displayNoTeachersMessage(VBox teacherContainer) {
    ...
    private void displayTeachers(List<Teacher> teachers, VBox teacherContainer) {
    ...
    private AnchorPane createTeacherCard(Teacher teacher) {
        ...
        // AIS ID
        Label idLabel = new Label(teacher.getId());
        idLabel.setStyle("-fx-font-weight: bold; -fx-font-size: 14px; -fx-text-fill: white;");
        card.getChildren().add(idLabel);
        AnchorPane.setTopAnchor(idLabel, 15.0);
        ...
        // Room
        Label roomLabel = new Label(teacher.getOffice());
        roomLabel.setStyle("-fx-font-size: 12px; -fx-text-fill: white;");
        card.getChildren().add(roomLabel);
        AnchorPane.setTopAnchor(roomLabel, 15.0);
        AnchorPane.setLeftAnchor(roomLabel, 675.0);
    ...
    private void openTeacherPage(Teacher teacher) {
        openModalWindow(
                AppConfig.getTEACHERS_SUB_PAGE_PATH(),
                "Teacher: " + teacher.getId(),
                "Error loading the teacher details window",
                teacher
        );
    }
    ...
    private void openModalWindow(String fxmlPath, String windowTitle, String errorMessage, Teacher teacher) {
        try {
            Stage parentStage = (Stage) btn_teacherspage.getScene().getWindow();
